...
#c.execute("DROP TABLE EndBossAnswers") #empty table
#conn.commit
#c.execute("CREATE TABLE IF NOT EXISTS Guests( Guest_id INTEGER PRIMARY KEY AUTOINCREMENT, First_name TEXT NOT NULL, Last_name TEXT NOT NULL, Email_address TEXT NOT NULL, Level INTEGER DEFAULT 1 NOT NULL, Username TEXT NOT NULL, Password TEXT NOT NULL);")
...
#c.execute("CREATE TABLE IF NOT EXISTS EndBossAnswers(Guest_id_f INTEGER, Guest_name TEXT, Date_and_Time TIMESTAMP, Answer TEXT, FOREIGN KEY(Guest_id_f) REFERENCES Guests(Guest_id))")
        ...
        if request.method == 'POST':
            ...
            if answer:
                ...
                if len(rows) > 0:
                    ...
                    now = datetime.datetime.now()
                    today = Calcdate(now)
                    c.execute("INSERT INTO EndBossAnswers (Guest_id_f, Guest_name,Date_and_Time, Answer) VALUES (?,?,?,?)",(id,user,today, answer) )
